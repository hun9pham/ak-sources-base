...
uint32_t flashGetSectorStartAddr(int16_t blId, int8_t secId) {
	if (blId < FLASH_BLOCK_ID_MIN && blId > FLASH_BLOCK_ID_MAX) {
		return 0;
	}
    ...
    return (uint32_t)(FLASH_BLOCK_START_ADDR + (blId * FLASH_BLOCK_SIZE)) + (secId * FLASH_SECTOR_SIZE);
}

uint32_t flashGetSectorEndAddr(int16_t blId, int8_t secId) {
    return ((flashGetSectorStartAddr(blId, secId) + FLASH_SECTOR_SIZE) - 0x00000001);
}

uint32_t flashGetBlockStartAddr(int16_t blId) {
    if (blId < FLASH_BLOCK_ID_MIN && blId > FLASH_BLOCK_ID_MAX) {
		return 0;
	}

    return (uint32_t)(FLASH_BLOCK_START_ADDR + (blId * FLASH_BLOCK_SIZE));
}

uint32_t flashGetBlockEndAddr(int16_t blId) {
    return ((flashGetBlockStartAddr(blId) + FLASH_BLOCK_SIZE) - 0x00000001);
}