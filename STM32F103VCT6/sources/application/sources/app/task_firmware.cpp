...
void TaskFirmware(ak_msg_t* msg) {
	...
	case SL_FIRMWARE_REPORT_STATUS: {
		...
		firmwareHeaderRead(&appFirmwareHeaderUpdate);
	...
	case SL_FIRMWARE_OTA_SETUP_REQ: {
		...
		for (uint8_t iDx = 0; iDx < NbrBlock64_Del; ++iDx) {
			flashEraseBlock64k(adtFLASH_FIRMWARE_UPDATE_CONTAINER_ADDR + (iDx * FLASH_BLOCK_SIZE));
		}
	...
	case SL_FIRMWARE_CALC_CHECKSUM_TRANSFER_REQ: {
		...
		for (uint32_t iDx = 0; iDx < newFirmwareLen; iDx += sizeof(uint32_t)) {
			watchdogRst();

			memset(&word, 0, sizeof(uint32_t));
			flashRead(adtFLASH_FIRMWARE_UPDATE_CONTAINER_ADDR + iDx, (uint8_t*)&word, sizeof(uint32_t));
			CsRaw += word;
		}
...
void entryUpdateBootSystem() {
	...
	uint32_t CsRaw = 0;
	uint16_t Cs_Calc;
	for (uint32_t iDx = 0; iDx < newBootUpdate.updateFirmwareBoot.binLen; iDx += sizeof(uint32_t)) {
		CsRaw += *((uint32_t*)(newBootUpdate.assert.desAddr + iDx));
	}
