 ...
 */
#include <stdbool.h>
#include "LcdGraphicLib.h"

// The following functions are not directly accessible to the application
static  void   LCD_PutChar8(uint8_t uc);
static  void   LCD_PutChar16(uint8_t uc);
static  void   LCD_PutChar16_ruff(uint8_t uc);
static  void   LCD_PutChar16_smooth(uint8_t uc);
static  void   LCD_PutChar12(uint8_t uc);
static  void   LCD_PutChar24(uint8_t uc);
...
// Private data...
static  uint16_t screenBuffer[64][8];     // 64 rows x 8 cols x 16 pixels

static  uint8_t  PixelMode;     // Pixel writing mode (see LCD_SetMode fn)
...
static  uint8_t  FontSize;      // Char cell height (pixels)
 ...
 * Input              :  x, y coords
------------------------------------------------------------------------------------*/
void  LCD_PosXY(uint16_t x, uint16_t y)
{
    CursorPosX = x;
    CursorPosY = y;
}
 ...
 * Return             :  --
------------------------------------------------------------------------------------*/
void   LCD_BlockFill(uint16_t w, uint16_t h)
{
    ...
    LCD_WriteBlock((uint16_t *) screenBuffer, x, y, w, h);
}
...
------------------------------------------------------------------------------------*/
uint8_t   LCD_PutImage(bitmap_t *imageData, uint16_t w, uint16_t h)
{
 ...
 * 
------------------------------------------------------------------------------------*/
void   LCD_SetFont(uint8_t font_ID)
{
 ...
 * Return             :  --
 --------------------------------------------------------------------------------------*/
void   LCD_PutDigit(uint8_t bDat)
{
    bDat &= 0x0F;
 ...
 * Return             :  --
 ---------------------------------------------------------------------------------------*/
void   LCD_PutHexByte(uint8_t bDat)
{
...
*
*--------------------------------------------------------------------------------------*/
void  LCD_PutDecimalWord(uint16_t uwVal, uint8_t bFieldSize)
{
...
static  void   LCD_PutChar16_ruff(uint8_t uc)
{
    ...
    LCD_PutImage((uint8_t *) bitmap, 12, 16);
}
...
static  void  LCD_PutChar24(uint8_t uc)
{
    ...
    LCD_PutImage((uint8_t *) bitmap, 16, 24);
