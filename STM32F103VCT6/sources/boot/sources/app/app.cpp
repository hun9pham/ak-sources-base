...
int boot_app() {
			 ...
			 mcuSysBoot.assert.cmdUpdate == sysBOOT_CMD_UPDATE_APP_REQ)
	{
		...
		while (fwIndex <  mcuSysBoot.updateFirmwareApp.binLen) {
			...
			for (uint16_t iDx = 0 ; iDx < NUM_OF_BYTE_FIRMWARE_UPLOAD; ++iDx) {
				csRaw += (uint32_t)loadData[iDx];
			}
		...
		uint32_t CsRaw = 0;
		uint16_t CsCalc;
		for (uint32_t iDx = 0; iDx < mcuSysBoot.updateFirmwareApp.binLen; iDx += sizeof(uint32_t)) {
			CsRaw += *((uint32_t*)(mcuSysBoot.assert.desAddr + iDx));
		}
...
uint8_t firmwareAppBackup(void) {
	...
	for (uint32_t iDx = (uint32_t)&(_start_flash); iDx < ((uint32_t)&(_start_flash) + flashLen); iDx += sizeof(uint32_t)) {
		...
		memcpy(&buf, ((uint32_t*)iDx), sizeof(uint32_t));
	...
	for (uint32_t iDx = 0; iDx < flashLen; iDx += sizeof(uint32_t)) {
		watchdogRst();

		word = 0;
		flashRead(FLASH_FIRMWARE_BACKUP_ADDR + iDx, (uint8_t*)&word, sizeof(uint32_t));
		CsRawCompareTo += word;
	}
