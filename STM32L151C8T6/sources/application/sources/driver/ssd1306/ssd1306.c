...
/* Write command */
#define SSD1306_WRITE_CMD(command)      SSD1306_I2C_WriteCommand(command)
/* Write data */
#define SSD1306_WRITE_DATA(data)        SSD1306_I2C_WriteData(data)
/* Absolute value */
...
void SSD1306_ScrollRight(uint8_t start_row, uint8_t end_row) {
	SSD1306_WRITE_CMD (SSD1306_RIGHT_HORIZONTAL_SCROLL);  // send 0x26
	SSD1306_WRITE_CMD (0x00);  // send dummy
	SSD1306_WRITE_CMD(start_row);  // start page address
	SSD1306_WRITE_CMD(0X00);  // time interval 5 frames
	SSD1306_WRITE_CMD(end_row);  // end page address
	SSD1306_WRITE_CMD(0X00);
	SSD1306_WRITE_CMD(0XFF);
	SSD1306_WRITE_CMD (SSD1306_ACTIVATE_SCROLL); // start scroll
}


void SSD1306_ScrollLeft(uint8_t start_row, uint8_t end_row) {
	SSD1306_WRITE_CMD (SSD1306_LEFT_HORIZONTAL_SCROLL);  // send 0x26
	SSD1306_WRITE_CMD (0x00);  // send dummy
	SSD1306_WRITE_CMD(start_row);  // start page address
	SSD1306_WRITE_CMD(0X00);  // time interval 5 frames
	SSD1306_WRITE_CMD(end_row);  // end page address
...
void SSD1306_Scrolldiagright(uint8_t start_row, uint8_t end_row) {
	SSD1306_WRITE_CMD(SSD1306_SET_VERTICAL_SCROLL_AREA);  // sect the area
	SSD1306_WRITE_CMD (0x00);   // write dummy
	SSD1306_WRITE_CMD(SSD1306_HEIGHT);

	SSD1306_WRITE_CMD(SSD1306_VERTICAL_AND_RIGHT_HORIZONTAL_SCROLL);
	SSD1306_WRITE_CMD (0x00);
	SSD1306_WRITE_CMD(start_row);
	SSD1306_WRITE_CMD(0X00);
	SSD1306_WRITE_CMD(end_row);
	SSD1306_WRITE_CMD (0x01);
	SSD1306_WRITE_CMD (SSD1306_ACTIVATE_SCROLL);
}
...
void SSD1306_Scrolldiagleft(uint8_t start_row, uint8_t end_row) {
	...
	SSD1306_WRITE_CMD(SSD1306_VERTICAL_AND_LEFT_HORIZONTAL_SCROLL);
	SSD1306_WRITE_CMD (0x00);
	SSD1306_WRITE_CMD(start_row);
	SSD1306_WRITE_CMD(0X00);
	SSD1306_WRITE_CMD(end_row);
...
void SSD1306_UpdateScreen(void) {
	...
	uint16_t i = 0;
	for (m = 0; m < 8; m++) {
		SSD1306_WRITE_CMD(0xB0 + m);
		SSD1306_WRITE_CMD(0x00);
		SSD1306_WRITE_CMD(0x10);
		
		SSD1306_I2C_StartWriteAccess();
		SoftI2c_WriteByte(0x40);

		for(n = 0; n < SSD1306_WIDTH; n++) {
			SoftI2c_WriteByte(SSD1306_Buffer[i++]);
		}
		SSD1306_I2C_StopAccess();
	}
}
...
//
/////////////////////////////////////////////////////////////////////////////////////////////////////////
void SSD1306_I2C_StartWriteAccess(void) {
...
void SSD1306_I2C_StopAccess(void) {
