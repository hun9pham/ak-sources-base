 ...
 * > Date: 23/12/2022
 * > Modify:
 *      . Updating new broadcast address is 255
 *      . Which updateing:
 *              Line 758
 *              Line 86
 * ****************************************************************************
...
#define MBM_SER_PDU_PDU_OFF             ( 1 )   /*!< Offset of csModbus-PDU in RTU frame. */
...
eMBMSerialRTUFrameReceive( xMBHandle xHdl, UCHAR ucSlaveAddress, USHORT * pusMBPDULength )
{
...
#endif
    {
        pxRTUHdl = pxIntHdl->xFrameHdl;
        if( MB_IS_VALID_HDL( pxRTUHdl, xMBMRTUFrameHdl ) && 
          ( MB_IS_VALID_READ_ADDR( ucSlaveAddress ) || 
            ucSlaveAddress == MB_SER_BROADCAST_ADDR || 
            ucSlaveAddress == MB_SER_BROADCAST_ADDR_ADD ) 
          )
        {
            if( NULL == pusMBPDULength )
            {
                ...
                xAnalyzerFrame.eFrameType = MB_FRAME_DAMAGED;
                    ...
                    //  ( ucSlaveAddress == pxRTUHdl->ubRTUFrameBuffer[MBM_SER_PDU_ADDR_OFF] || ucSlaveAddress == MB_SER_BROADCAST_ADDR || ucSlaveAddress == MB_SER_BROADCAST_ADDR_ADD ) &&
...
bMBMSerialRTUTransmitterEmptyAPIV1CB( xMBHandle xHdl, UBYTE * pubValue )
{
    ...
    case MBM_STATE_TX_XMIT:
        ...
        else
        {
            ...
            //     MB_SER_BROADCAST_ADDR_ADD == pxRTUFrameHdl->ubRTUFrameBuffer[MBM_SER_PDU_ADDR_OFF] )
            // {
            ...
            //
            //=============================================================
            // HungPNQ: Modify app modbus master always received respond
            //=============================================================
            bEnableRx = TRUE;
        }
        break;
