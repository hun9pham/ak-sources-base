               ...
               pbMBPTimerExpiredCB pbMBPTimerExpiredFN, xMBHandle xHdl )
{
    ...
    if( ( NULL != xTimerHdl ) && ( NULL != pbMBPTimerExpiredFN ) && ( MB_HDL_INVALID != xHdl ) )
    {
        if( !bIsInitalized )
        {
            ...
            TIM_TimeBaseInit( MB_TIM, &TIM_TimeBaseStructure );

            /* Enable MB_TIMER x interrupt */
            NVIC_InitStructure.NVIC_IRQChannel = MB_TIM_IRQn;
            NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = MB_PREEMP_PRIORITY;
            NVIC_InitStructure.NVIC_IRQChannelSubPriority = 0;
            NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;
            NVIC_Init( &NVIC_InitStructure );

            TIM_SetCounter(MB_TIM, 0);
            TIM_ClearITPendingBit(MB_TIM, TIM_IT_Update);
	        TIM_ClearFlag(MB_TIM, TIM_IT_Update);

            /* Enable TIMER x interrupt */
            TIM_ITConfig( MB_TIM, TIM_IT_Update, ENABLE );

            /* Enable TIMER x */
            TIM_Cmd( MB_TIM, ENABLE );
...
vMBPTimerISR( void )
{
    ...
    if( TIM_GetITStatus( MB_TIM, TIM_IT_Update ) != RESET )
    {
#if MBP_DEBUG_TIMER_PERFORMANCE == 1
