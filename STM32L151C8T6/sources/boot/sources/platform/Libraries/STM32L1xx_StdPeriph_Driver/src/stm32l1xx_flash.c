...
/**
  * @brief  Configures to app from Bank1 or Bank2.
  * @note   This function can be used only for STM32L1XX_HD and STM32L1XX_XL devices.
  * @param  OB_BOOT: select the FLASH Bank to app from.
  *   This parameter can be one of the following values:
  *     @arg OB_BOOT_BANK2: At startup, if app pins are set in app from user Flash
  *        position and this parameter is selected the device will app from Bank2 or Bank1,
  *        depending on the activation of the bank. The active banks are checked in
  *        the following order: Bank2, followed by Bank1.
  *        The active bank is recognized by the value programmed at the base address
  *        of the respective bank (corresponding to the initial stack pointer value
  *        in the interrupt vector table).
  *     @arg OB_BOOT_BANK1: At startup, if app pins are set in app from user Flash
  *        position and this parameter is selected the device will app from Bank1(Default).
